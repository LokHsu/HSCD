...
from .pareto_optim import pareto_backward
...
class Trainer:
    ...
    def train_epoch(self, pretrain=False):
        ...
        with tqdm(total=len(train_loader), desc='Training', unit='batch', leave=False) as pbar:
            for user_indices, pos_indices, neg_indices in train_loader:
                ...
                main_loss, aux_losses = self.model.loss(user_indices, pos_indices, neg_indices) if not pretrain else self.model.pretrain_loss(user_indices, pos_indices, neg_indices)

                self.optimizer.zero_grad()
                w = pareto_backward(self.model, main_loss, aux_losses, lam=self.args.lam)
                self.optimizer.step()

                loss = main_loss
                for w_t, loss_t in zip(w, aux_losses.values()):
    ...
    def train_model(self):
        ...
        for epoch in pbar:
            ...
            if self.args.wandb:
                wandb_logger.log_metrics(epoch, loss, {'hr': hr, 'ndcg': ndcg})
